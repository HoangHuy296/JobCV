...
const CompanyListing: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCompanies = async () => {
      try {
        setLoading(true);
        const response = await getAllCompanies(currentPage, companiesPerPage, activeSearchTerm, selectedIndustry?.toString() || '');
    ...
    fetchCompanies();
  }, [currentPage, activeSearchTerm, selectedIndustry]);
  ...
  const handleIndustryFilter = useCallback((industryId: number) => {
