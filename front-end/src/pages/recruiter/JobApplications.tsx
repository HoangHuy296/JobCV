...
const JobApplications: React.FC = () => {
  ...
  const fetchJobAndApplications = async () => {
    try {
      ...
      // Fetch job details
      const jobResponse = await getJobById(Number(jobId));
      setJob(jobResponse);
      
      // Fetch applications for this job
      const applicationsResponse = await getJobApplications(Number(jobId));
  ...
  const handleViewCV = (cvId: number | null) => {
    if (!cvId) {
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">
                  Quản lý hồ sơ ứng tuyển • {applications.length} ứng viên
                </p>
              </div>
            </div>
          </div>
